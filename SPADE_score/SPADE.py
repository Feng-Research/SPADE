...
def GetRiemannianDist(Gx, Gy, num_eigs, gnd, one_class, julia):
    ...
    if julia:
        ...
        [Dxy, Uxy] = Main.main(l_in, l_out, k)
    else:
...
def Spade(data_input, data_output, data_labels, k, num_eigs, massive_data=True, full_function=True, The_faiss=True, graph=False, one_class=False, julia = False): 
    import numpy as np
    import pandas as pd
    import networkx as nx
    ...
    if full_function:
        TopEig, TopEdgeList, TopNodeList = GetRiemannianDist(Gx, Gy, num_eigs, data_labels,one_class, julia)# full function
        t.stop()
        if graph:
        ...
        input_ = input_.asfptype()# use it if choose hnsw(data, k)
        output_ = output_.asfptype()#use it if choose hnsw(data, k)
        if julia:
            from julia.api import Julia
            jl = Julia(compiled_modules=False)
            from julia import Main
