"""
The code is adapted from the Jingfeng Zhang's code:
https://github.com/zjfheart/Geometry-aware-Instance-reweighted-Adversarial-Training/blob/main/GAIRAT.py
"""
...
def train(epoch, end_epoch, model, train_loader, test_loader, optimizer, Lambda):
    
    lr = 0
    num_data = 0
    train_robust_loss = 0

    x_adv_res = np.zeros((1,3*32*32))
    data_res = np.zeros((1,3*32*32))
    kappa_res = np.zeros((1))
    logit_res = np.zeros((1,10))
    logit_clean_res = np.zeros((1,10))
    weight_res = np.zeros((1))
    labels_res = np.zeros((1))
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if len(shp) == 2:
            ...
            x_adv_out = x_adv.reshape(shp[0], shp[1]*shp[2]*shp[3])
            data_out = data.reshape(shp[0], shp[1]*shp[2]*shp[3])
        x_adv_res  = np.vstack((x_adv_res, x_adv_out.detach().cpu().numpy()))  
        kappa_res = np.append(kappa_res, Kappa.cpu().numpy())      
        logit_res = np.vstack((logit_res, logit.detach().cpu().numpy()))
        data_res = np.vstack((data_res, data_out.detach().cpu().numpy()))  
        logit_clean_res = np.vstack((logit_clean_res, logit_clean.detach().cpu().numpy()))
        labels_res = np.append(labels_res,target.cpu().numpy())
        ...
        if (epoch + 1) >= args.begin_epoch:
            ...
            weight_res = np.append(weight_res, normalized_reweight.cpu().numpy())
    ...
    x_adv_res = x_adv_res[1:]
    kappa_res = kappa_res[1:]
    logit_res = logit_res[1:]
    logit_clean_res = logit_clean_res[1:]
    data_res = data_res[1:]
    weight_res = weight_res[1:]
    labels_res = labels_res[1:]

    return train_robust_loss, lr, x_adv_res, kappa_res, logit_res, data_res, logit_clean_res, weight_res, labels_res
...
if args.dataset == "mnist":
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=False, num_workers=1,pin_memory=True)
...
best_x_adv_res = np.array([])
best_kappa_res = np.array([])
best_logit_res = np.array([])
best_data_res = np.array([])
best_logit_clean_res = np.array([])
best_weight_res = np.array([])
best_labels_res = np.array([])

output_ad = r'{}/'.format(args.out_dir)
if not os.path.exists(output_ad):
    os.makedirs(output_ad)
...
for epoch in range(start_epoch, args.epochs):
    ...
    train_robust_loss, lr, x_adv_res, kappa_res, logit_res, data_res, logit_clean_res, weight_res, labels_res = train(epoch, args.epochs, model, train_loader, test_loader, optimizer, Lambda)
    ...
    if test_pgd20_acc > best_acc:
        ...
        best_x_adv_res = x_adv_res
        best_kappa_res=kappa_res
        best_logit_res = logit_res
        best_data_res = data_res
        best_logit_clean_res = logit_clean_res
        best_weight_res = weight_res
        best_labels_res = labels_res
    ...
    np.savetxt("{}best_x_adv.csv".format(output_ad),best_x_adv_res, "%.3f")
    np.savetxt("{}best_kappa.csv".format(output_ad),best_kappa_res, "%.1f")
    np.savetxt("{}best_logit.csv".format(output_ad),best_logit_res, "%.3f")
    np.savetxt("{}best_logit_clean.csv".format(output_ad),best_logit_clean_res, "%.3f")
    np.savetxt("{}best_data.csv".format(output_ad),best_data_res, "%.3f")
    np.savetxt("{}best_weight_res.csv".format(output_ad),best_weight_res, "%.3f")
    np.savetxt("{}best_labels_res.csv".format(output_ad),best_labels_res, "%.3f")
logger_test.close()
